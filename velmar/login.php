...
<?php 
    ...
    if (isset($_POST["user"])&& isset($_POST["pwd"])) :
      ...
      $user=$_POST["user"];
      $pwd=$_POST["pwd"];
            //consulra para ver si esta el usuario registrado

      $sqlp = $db->prepare("SELECT * FROM usuario 
                 WHERE usuario=:user AND password=MD5(:pwd) ;") ;
    $sqlp->bindParam(":user",$user,PDO::PARAM_STR,10) ;
    $sqlp->bindParam(":pwd",$pwd,PDO::PARAM_STR,32) ;
      if ($sqlp->execute()):
      if ($sqlp->rowCount() > 0):
                //si esta registrado le enviamo a inicio.php

        
          header("location:http://localhost/velmar/libs/inicio.php");
...
<body>
  ...
  <a id="bo" href="libs/registro.php" style="position: absolute;top: 310px;left: 300px;">registrate</a>
</form>
</div>
